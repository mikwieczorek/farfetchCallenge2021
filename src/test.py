...
def get_parser():
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="model_trained.ckpt",
        help="Path to checkpoint file",
    )
    parser.add_argument(
        "--subset-to-use", type=str, default="test", help="Either val or test"
    )
    return parser
...
def main(params: dict):
    ...
    if params.subset_to_use == "test":
        test_loader = test_original_loader
    elif params.subset_to_use == "val":
        test_loader = test_original_loader
    else:
        raise ValueError

    trainer.test(
        model, test_dataloaders=[test_loader], ckpt_path=params.checkpoint_path
    )
